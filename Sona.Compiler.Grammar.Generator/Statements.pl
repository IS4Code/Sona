...
analyze_trail(Analyzer, Predicate, Result) :- call(Analyzer, special_blocks, Predicate, Result).

run :-
  analyze_singleton(analyze_do, is_terminating, DoTerminating),
  analyze_trail(analyze_do, is_returning, DoReturning),
  analyze_trail(analyze_do, is_conditional, DoConditional),
  save('s_do.json', result{terminating:DoTerminating, returning:DoReturning, conditional:DoConditional}),
  
  analyze_trail(analyze_while, is_returning, WhileReturning),
  analyze_trail(analyze_while, is_conditional, WhileConditional),
  save('s_while.json', result{returning:WhileReturning, conditional:WhileConditional}),
  
  analyze_singleton(analyze_while_true, is_terminating, WhileTrueTerminating),
  analyze_trail(analyze_while_true, is_returning, WhileTrueReturning),
  analyze_trail(analyze_while_true, is_conditional, WhileTrueConditional),
  save('s_while_true.json', result{terminating:WhileTrueTerminating, returning:WhileTrueReturning, conditional:WhileTrueConditional}),
  
  analyze_singleton(analyze_if, is_terminating, IfTerminating),
  analyze_trail(analyze_if, is_returning, IfReturning),
  analyze_trail(analyze_if, is_conditional, IfConditional),
  save('s_if.json', result{terminating:IfTerminating, returning:IfReturning, conditional:IfConditional}).
